 ...
 * grabbing the next data set. I next decided instead of trying a costly
 * simulation I would pick one light to test off of but only when that light is
 * green. I decided to do the least amount of testing I would do it on the one
 * with the smallest interval time since I only test for when it's green and a
 * good amount of that will be taken by the 5 seconds it spends yellow of which
 * I will not test for. One good reason for doing this is all the lights start
 * as green and we only care about the next time they become green after the
 * first yellow light which is gonna be on the smallest interval.
 *
 * ------------------------ Processing the Data ---------------------------
 ...
 * if the other lights are green by dividing the current time by that lights
 * interval and truncating it. Since we start with the lowest interval we know
 * the division will never break out of the interval it is in be it green or red
 * so we can truncate it and only worry about if it is in a red or green
 * interval current, since the lights start out green we can the following
 * logic, if the number is even it matches the the starting value of green. If
 * it is odd it is not and is thus red. Now that just checks if we're in a red
 * or green interval, lastly we mod the number by the interval and make sure the
 * remainder is not more then the interval minus 5 to check for yellow lights.
...
int main() {
  ...
  // if a data set starts with a 0 it is actaully then end of file marker. so
  // here we cin during our while loop test to make sure we can because if we
  // can't then we have for sure reached the end of the redirected and then we
  // test for the end of file sentenal value of 0 by treating the cined value as
  // a logical varible
    ...
    while (timeFound == false && currentIntervalStartTime <= 18000) {
      ...
      for (int i = 0; i < (lights[0] - 5) && timeFound == false; i++) {
        ...
        for (int j = 1; j < lights.size(); j++) {
          ...
          if (floor % 2 != 0 || remainder > lights[j] - 6) {
            timeFound = false;
          }
        }
        if (timeFound) {
